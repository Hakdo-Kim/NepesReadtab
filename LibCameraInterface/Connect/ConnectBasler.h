...
#include "Connect.h"
#include "Transport/LibBasler.h"

#ifdef ENABLE_CAMERA_LIBRARY_BASLER

class CConnectBasler : public CConnect
{
public:
	CConnectBasler();
	virtual ~CConnectBasler();
	...
	virtual BOOL GetGain(const int nIdx, const int nGainType, int* pnGainValue);
	...
	virtual BOOL GetFrameRate(const int nIdx, unsigned int* pnFrameRate);
	...
	virtual BOOL GetExposureTime(const int nIdx, unsigned int* pnSetExposureTime);
	...
	virtual BOOL GetTriggerMode(const int nIdx, INFO_TRIGGER_PARA* pstTrigger);
	...
	virtual BOOL GetStrobeMode(const int nIdx, INFO_STROBE_PARA* pstStrobe);

	virtual BOOL SetFlipMode(const int nIdx, unsigned short nMode);
	virtual BOOL GetFlipMode(const int nIdx, unsigned short* pnMode);

	virtual BOOL SetROI(const int nIdx, INFO_ROI_PARA stROI);
	virtual BOOL GetROI(const int nIdx, INFO_ROI_PARA* pstROI);

	virtual BOOL SetUsersetStartNo(const int nIdx, unsigned char nUsersetStartNo);
	virtual BOOL GetUsersetStartNo(const int nIdx, unsigned char* pnUsersetStartNo);
	...
	virtual BOOL SetColorFormat(const int nIdx, TYPE_CAMCOLORFORMAT eFormat);		// GR, RG, GB, BG
	virtual BOOL GetColorFormat(const int nIdx, TYPE_CAMCOLORFORMAT* peFormat);
...
protected:
	CLibBasler m_LibBasler;
	//INFO_ARG m_ArgList[MAX_INTERFACE_MODULE];
};
