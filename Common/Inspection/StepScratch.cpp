...
CStepScratch::CStepScratch()
	: CStep("SCRATCH")
{
	memset(&m_stInfoScratch, 0x00, sizeof(INFO_INSPECTION_SCRATCH));
	for (int i = 0; i < MAX_STEPSTROBE_LIST; i++)
	{
		m_stInfoScratch.m_pstStepStrobe[i].m_eStrobeType = TYPE_STROBE_OFF;
		m_stInfoScratch.m_pstStepStrobe[i].m_nStrobeBrightness = 0;
	}
...
BOOL CStepScratch::Init(TYPE_CAMERAPOS eCameraPos, INFO_LOGFUNC stLogFunc)
{
	BOOL bResult = TRUE;
	Deinit();
...
BOOL CStepScratch::Process(CSampleSide& sample)
{
	BOOL bResult = TRUE;
		...
		sample.m_pRefImage->m_stFrame.m_eDataBit != TYPE_DATABIT_08)
	{
		...
		camera_input.copyTo(sample.m_ScratchResult.m_pProbImage);
		if (sample.m_ScratchResult.m_pProbImage.channels() == 1)
			cv::cvtColor(sample.m_ScratchResult.m_pProbImage, sample.m_ScratchResult.m_pProbImage, CV_GRAY2BGR);
	...
	roi.x = m_stInfoScratch.m_stRoi.m_rtRoi.left;
	if (roi.x < 0){
		roi.x = 0;
	}
	...
	roi.y = m_stInfoScratch.m_stRoi.m_rtRoi.top;
	if (roi.y < 0){
		roi.y = 0;
	}
	...
	if (NAVIAIError_Success != result){
		...
		input2.copyTo(sample.m_ScratchResult.m_pProbImage);
		if (sample.m_ScratchResult.m_pProbImage.channels() == 1)
			cv::cvtColor(sample.m_ScratchResult.m_pProbImage, sample.m_ScratchResult.m_pProbImage, CV_GRAY2BGR);
	...
	threshold(g_result_img, threshold_output, m_stInfoScratch.m_stParaScratch.prob_thres, 1, cv::THRESH_BINARY);

	// Log Test
	//  defect size check
	bResult = checkBlobSize(threshold_output, g_result_img, sample.m_ScratchResult.m_fMaxDefectSize, m_stInfoScratch.m_stParaScratch.blobSize_thres, 5, m_stLogFunc, m_eCameraPos);
	...
	for (int y = roi.y; y < roi.y + roi.height; y++) {
		for (int x = roi.x; x < roi.x + roi.width; x++) {
			...
			if (value > m_stInfoScratch.m_stParaScratch.prob_thres){
	...
	cv::resize(g_view_img, sample.m_ScratchResult.m_pProbImage, size);

#endif

	sample.m_ScratchResult.m_bResult = bResult;
	Log("Process End");
	return TRUE;
}
BOOL CStepScratch::Process(CSampleSide& sample, cv::Mat captureImg)
{
	BOOL bResult = TRUE;
	...
	sample.m_ScratchResult.m_pInputImage = captureImg;
	cv::Mat input2;
	//  input2 : same size with camera input
#ifdef USE_BASLER_CAMERA
	//input2 = cv::Mat(sample.m_pRefImage->m_stFrame.m_dwHeight, sample.m_pRefImage->m_stFrame.m_dwWidth, CV_8UC3, sample.m_pRefImage->m_pBuffer, sample.m_pRefImage->m_stFrame.m_dwPitch);
...
BOOL CStepScratch::Load(INFO_INSPECTION* pstInspection)
{
	BOOL bResult = FALSE;
	if (pstInspection)
	{
		bResult = SetInformation(&pstInspection->m_stScratch);
	}
	else
	{
		// Default setting
		m_stInfoScratch.m_pstStepStrobe[0].m_eStrobeType = TYPE_STROBE_RING;
		m_stInfoScratch.m_pstStepStrobe[0].m_nStrobeBrightness = 15;
		m_stInfoScratch.m_pstStepStrobe[1].m_eStrobeType = TYPE_STROBE_OFF;
		m_stInfoScratch.m_pstStepStrobe[1].m_nStrobeBrightness = 0;


		// Roi
		m_stInfoScratch.m_stRoi.m_eTrackerType = TYPE_TRACKER_RECT;
		m_stInfoScratch.m_stRoi.m_rtRoi.left = ROI_S;
		m_stInfoScratch.m_stRoi.m_rtRoi.top = ROI_S;
		m_stInfoScratch.m_stRoi.m_rtRoi.right = ROI_E;
		m_stInfoScratch.m_stRoi.m_rtRoi.bottom = ROI_E;
...
BOOL CStepScratch::Get(INFO_INSPECTION* pstInspection)
{
	if (NULL == pstInspection)
		return FALSE;

	pstInspection->m_stScratch = m_stInfoScratch;
	return TRUE;
}

BOOL CStepScratch::Set(const INFO_INSPECTION* pstInspection)
{
	if (NULL == pstInspection)
		return FALSE;

	SetInformation(&pstInspection->m_stScratch);
	return TRUE;
}

const INFO_INSPECTION_SCRATCH* CStepScratch::GetInformation()
{
	return &m_stInfoScratch;
}

BOOL CStepScratch::SetInformation(const INFO_INSPECTION_SCRATCH* pInfoSize)
{
	if (NULL == pInfoSize)
		return FALSE;

	memcpy(&m_stInfoScratch, pInfoSize, sizeof(INFO_INSPECTION_SCRATCH));
	return TRUE;
}