...
const int CStepSize::m_iDefaultDirection[TYPE_FINDLINE_MAX] = { 0, 2, 1, 3, 0, 3, 1, 0, 0, 0, 1, 1, 1, 0, 2, 1 };
const int CStepSize::m_iDefaultPolarity[TYPE_FINDLINE_MAX] = { 2, 2, 2, 2, 2, 2, 2, 0, 0, 0, 0, 0, 0, 2, 2, 2 };
const int CStepSize::m_iDefaultLookFor[TYPE_FINDLINE_MAX] = { 0, };
const int CStepSize::m_iDefaultStrengthThreshold[TYPE_FINDLINE_MAX] = { 60, 60, 60, 60, 10, 10, 10, 20, 20, 20, 20, 20, 20, 10, 10, 10 };
const int CStepSize::m_iDefaultSampling[TYPE_FINDLINE_MAX] = { 50, 50, 50, 50, 10, 10, 10, 20, 20, 20, 20, 20, 20, 10, 10, 10 };
const int CStepSize::m_iDefaultKernelSize[TYPE_FINDLINE_MAX] = { 3, 3, 3, 3, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7, 7 };
...
*/
BOOL CStepSize::Init(TYPE_CAMERAPOS eCameraPos, INFO_LOGFUNC stLogFunc)
{
	BOOL bResult = TRUE;
...
* @return 검사 완료 여부
*/
BOOL CStepSize::Process(CSampleSide& sample)
{
	...
	if (!DongleCheck())
	{
		...
		if (!DongleLogin())
		{
			...
			memcpy(&sample.m_SizeResult.m_stCalibratedImage, &img, sizeof(stAPPIMAGEDATA));
	...
	stLine *stResultLine = sample.m_SizeResult.m_stResultLine;
	stPOINT *stResultPoint = sample.m_SizeResult.m_stResultPoint;
	...
	// Geometry (Distance)
	OUTPUT_DISTANCE *stDistance = sample.m_SizeResult.m_stDistance;
	stLine *stDistanceLine = sample.m_SizeResult.m_stDistanceLine;
	...
	memcpy(&sample.m_SizeResult.m_stCalibratedImage, &outputImg, sizeof(stAPPIMAGEDATA));
	...
	for (int i = TYPE_FINDLINE_METAL_TOP; i <= TYPE_FINDLINE_METAL_LEFT; i++)
	{
		...
		if (roi.size() > 0 && roi.at(0).m_pstPosArray.size() >= 4)
		{
			for (int j = 0; j < 4; j++)
			{
				sample.m_SizeResult.m_stROIPoint[i].m_pt[j] = m_stCurruntROIPoint[i].m_pt[j] = roi.at(0).m_pstPosArray.at(j);
			}
		}
	}
...
BOOL CStepSize::Process(CSampleSide& sample, cv::Mat captureImg)
{
	auto starttime = chrono::high_resolution_clock::now();
	...
	if (!DongleCheck())
	{
		...
		if (!DongleLogin())
		{
			...
			memcpy(&sample.m_SizeResult.m_stCalibratedImage, &img, sizeof(stAPPIMAGEDATA)); // TODO: 버퍼 안정성 검토
	...
	if (bFindLineResult[L_M_T] && bFindLineResult[L_M_L])
	{
		...
		for (int i = TYPE_FINDLINE_METAL_TOP; i < TYPE_FINDLINE_MAX; i++)
			sample.m_SizeResult.m_bFindLineResult[i] = bFindLineResult[i];
...
*/
BOOL CStepSize::Load(INFO_INSPECTION* pstInspection)
{
	BOOL bResult = FALSE;
	if (pstInspection)
	{
		// Load
		bResult = SetInformation(&pstInspection->m_stSize);
	}
	else
	{
...
*/
BOOL CStepSize::Get(INFO_INSPECTION* pstInspection)
{
	if (NULL == pstInspection)
		return FALSE;

	memcpy(&pstInspection->m_stSize, &m_stInfoSize, sizeof(INFO_INSPECTION_SIZE));
	return TRUE;
}
...
* @remark 이 함수는 쓰이지 않을 수 있음
*/
BOOL CStepSize::Set(const INFO_INSPECTION* pstInspection)
{
	if (NULL == pstInspection)
		return FALSE;

	return SetInformation(&pstInspection->m_stSize);
}
...
* @return 설정 값 구조체 포인터
*/
const INFO_INSPECTION_SIZE* CStepSize::GetInformation()
{
	return &m_stInfoSize;
}
...
*/
BOOL CStepSize::SetInformation(const INFO_INSPECTION_SIZE* pInfoSize)
{
...
*/
BOOL CStepSize::SetInfoFindLineROI(const INFO_FINDLINE_ROI* pInfoLine, const INFO_IMAGE* pRefImage, const int iIndex)
{
...
* @remark 설정 값을 가져오고 모든 FindLine 객체의 설정 값 변경
*/
BOOL CStepSize::SetInfoFindLineParameter(const INFO_FINDLINE_PARAMETER* pInfoLine, const int iIndex)
{
...
* @return 설정 성공 여부
*/
BOOL CStepSize::SetFindLineAppParameter(CLPApp *app, const INFO_FINDLINE_PARAMETER info)
{
	if (NULL == app)
		return FALSE;
...
*/
BOOL CStepSize::SetFindLineAppROI(CLPApp *app, const INFO_FINDLINE_ROI *info, const bool enableRefROI, const stPOINT refPt, const double refAngle)
{
	if (NULL == app)
		return FALSE;
...
* @return 설정 성공 여부
*/
BOOL CStepSize::SetCalibration(const INFO_CALIBRATION* pstInfoCalibration)
{
...
* @return 설정 성공 여부
*/
BOOL CStepSize::SetSizeGain(const INFO_SIZE_GAIN* pstInfoSizeGain)
{
...
*/
BOOL CStepSize::LineIntersection(stAPPIMAGEDATA *img, const stLine &line1, const stLine &line2, stPOINT &ptOut)
{
...
* @remark 이미지는 사실상 필요 없음
*/
BOOL CStepSize::MidPoint(stAPPIMAGEDATA *img, const stPOINT &pt1, const stPOINT &pt2, stPOINT &ptOut)
{
	if (NULL == m_pGeometryApp || NULL == img)
		return FALSE;
...
*/
BOOL CStepSize::LinetoLineDistance(stAPPIMAGEDATA *img, const stLine &line1, const stLine &line2, double &distance, stLine &distanceLine)
{
	if (NULL == m_pGeometryApp || NULL == img)
		return FALSE;
...
* @remark Calibration App process 후 호출해야 함
*/
BOOL CStepSize::PointtoPointDistance(stAPPIMAGEDATA *img, const stPOINT &pt1, const stPOINT &pt2, double &distance, stLine &distanceLine)
{
	if (NULL == m_pGeometryApp || NULL == img)
		return FALSE;
...
* @remark Calibration App process 후 호출해야 함
*/
BOOL CStepSize::PointtoLineDistance(LP_VisionApp::stAPPIMAGEDATA *img, const stLine &line, const stPOINT &pt, double &distance, stLine &distanceLine)
{
	if (NULL == m_pGeometryApp || NULL == img)
		return FALSE;
...
* @remark Calibration App process 후 호출해야 함
*/
BOOL CStepSize::S1Distance(LP_VisionApp::stAPPIMAGEDATA *img, const stLine &metalLine, const stLine &tapeLineS1, const double metalLineDistance,
...
* @remark Calibration App process 후 호출해야 함
*/
BOOL CStepSize::S2Distance(LP_VisionApp::stAPPIMAGEDATA *img, const stLine &metalLine, const stLine &tapeLineS2, const double metalLineDistance,
...
* @remark Calibration App process 후 호출해야 함
*/
BOOL CStepSize::S3Distance(LP_VisionApp::stAPPIMAGEDATA *img, const stLine &metalLine, const stLine &tapeLineS3, const double metalLineDistance,
...
*/
void CStepSize::GetDefaultSizeSetting(INFO_INSPECTION_SIZE *info)
{
