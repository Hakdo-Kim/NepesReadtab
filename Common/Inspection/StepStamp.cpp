...
CStepStamp::CStepStamp()
	: CStep("STAMP")
{
	memset(&m_stInfoStamp, 0x00, sizeof(INFO_INSPECTION_STAMP));
	for (int i = 0; i < MAX_STEPSTROBE_LIST; i++)
	{
		m_stInfoStamp.m_pstStepStrobe[i].m_eStrobeType = TYPE_STROBE_OFF;
		m_stInfoStamp.m_pstStepStrobe[i].m_nStrobeBrightness = 0;
	}
...
BOOL CStepStamp::Init(TYPE_CAMERAPOS eCameraPos, INFO_LOGFUNC stLogFunc)
{
	BOOL bResult = TRUE;
	Deinit();
...
BOOL CStepStamp::Process(CSampleSide& sample)
{
	BOOL bResult = TRUE;
		...
		sample.m_pRefImage->m_stFrame.m_eDataBit != TYPE_DATABIT_08)
	{
		...
		camera_input.copyTo(sample.m_StampResult.m_pProbImage);
		if (sample.m_StampResult.m_pProbImage.channels() == 1)
			cv::cvtColor(sample.m_StampResult.m_pProbImage, sample.m_StampResult.m_pProbImage, CV_GRAY2BGR);
	...
	roi.x = m_stInfoStamp.m_stRoi.m_rtRoi.left;
	if (roi.x < 0){
		roi.x = 0;
	}
	...
	roi.y = m_stInfoStamp.m_stRoi.m_rtRoi.top;
	if (roi.y < 0){
		roi.y = 0;
	}
	...
	if (NAVIAIError_Success != result){
		...
		input2.copyTo(sample.m_StampResult.m_pProbImage);
		if (sample.m_StampResult.m_pProbImage.channels() == 1)
			cv::cvtColor(sample.m_StampResult.m_pProbImage, sample.m_StampResult.m_pProbImage, CV_GRAY2BGR);
	...
	threshold(g_result_img, threshold_output, m_stInfoStamp.m_stParaStamp.prob_thres, 1, cv::THRESH_BINARY);

	// Log Test
	//  defect size check
	bResult = checkBlobSize(threshold_output, g_result_img, sample.m_StampResult.m_fMaxDefectSize, m_stInfoStamp.m_stParaStamp.blobSize_thres, 6, m_stLogFunc, m_eCameraPos);
	...
	for (int y = roi.y; y < roi.y + roi.height; y++) {
		for (int x = roi.x; x < roi.x + roi.width; x++) {
			...
			if (value > m_stInfoStamp.m_stParaStamp.prob_thres){
	...
	cv::resize(g_view_img, sample.m_StampResult.m_pProbImage, size);

#endif

	sample.m_StampResult.m_bResult = bResult;
	Log("Process End");
	return TRUE;
}

BOOL CStepStamp::Process(CSampleSide& sample, cv::Mat captureImg)
{
	BOOL bResult = TRUE;
	...
	sample.m_StampResult.m_pInputImage = captureImg;
	cv::Mat input2;
	//  input2 : same size with camera input
#ifdef USE_BASLER_CAMERA
	//input2 = cv::Mat(sample.m_pRefImage->m_stFrame.m_dwHeight, sample.m_pRefImage->m_stFrame.m_dwWidth, CV_8UC3, sample.m_pRefImage->m_pBuffer, sample.m_pRefImage->m_stFrame.m_dwPitch);
...
BOOL CStepStamp::Load(INFO_INSPECTION* pstInspection)
{
	BOOL bResult = FALSE;
	if (pstInspection)
	{
		bResult = SetInformation(&pstInspection->m_stStamp);
	}
	else
	{
		// Default setting
		m_stInfoStamp.m_pstStepStrobe[0].m_eStrobeType = TYPE_STROBE_BAR;
		m_stInfoStamp.m_pstStepStrobe[0].m_nStrobeBrightness = 15;
		m_stInfoStamp.m_pstStepStrobe[1].m_eStrobeType = TYPE_STROBE_OFF;
		m_stInfoStamp.m_pstStepStrobe[1].m_nStrobeBrightness = 0;


		// Roi
		m_stInfoStamp.m_stRoi.m_eTrackerType = TYPE_TRACKER_RECT;
		m_stInfoStamp.m_stRoi.m_rtRoi.left = ROI_S;
		m_stInfoStamp.m_stRoi.m_rtRoi.top = ROI_S;
		m_stInfoStamp.m_stRoi.m_rtRoi.right = ROI_E;
		m_stInfoStamp.m_stRoi.m_rtRoi.bottom = ROI_E;
...
BOOL CStepStamp::Get(INFO_INSPECTION* pstInspection)
{
	if (NULL == pstInspection)
		return FALSE;

	pstInspection->m_stStamp = m_stInfoStamp;
	return TRUE;
}

BOOL CStepStamp::Set(const INFO_INSPECTION* pstInspection)
{
	if (NULL == pstInspection)
		return FALSE;

	SetInformation(&pstInspection->m_stStamp);
	return TRUE;
}

const INFO_INSPECTION_STAMP* CStepStamp::GetInformation()
{
	return &m_stInfoStamp;
}

BOOL CStepStamp::SetInformation(const INFO_INSPECTION_STAMP* pInfoSize)
{
	if (NULL == pInfoSize)
		return FALSE;

	memcpy(&m_stInfoStamp, pInfoSize, sizeof(INFO_INSPECTION_STAMP));
	return TRUE;
}