...
BOOL CSerialListener::StartListener(LPCTSTR lpszDevice, DWORD dwBaudrate, DWORD dwId, HWND hWnd, FuncNotifySerialListener pFunc, void* pArg)
{
	if (NULL == lpszDevice)
		return FALSE;
...
DWORD CSerialListener::ThreadProc(void)
{
	...
	do
	{
		...
		else
		{
			...
			do
			{
				BYTE pBuffer[MAX_LISTEN_LEN] = { 0, };
				DWORD dwBuf = 0;
				if (FALSE == this->Read(pBuffer, MAX_LISTEN_LEN, &dwBuf) || 1>dwBuf)
				{
					break;
				}
			...
			if (0 < nReadIdx && '\r' == pRead[nReadIdx - 1])
			//if (0 < nReadIdx && '\r' == pRead[nReadIdx - 2] && '\n' == pRead[nReadIdx - 1])
			{
				int nPos = 0;
				memset(m_szReadBuffer, 0x00, MAX_PATH);
				...
				if (m_FuncNotify && m_pArg)
				{
					/*//
					{
						TCHAR szText[MAX_SERIALNAME_LEN] = { 0, };
						CUtil::StringFormat(szText, MAX_SERIALNAME_LEN, _T("[%d] %s"), ::GetTickCount(), m_szReadBuffer);
						CUtil::PrintDebugString(szText);
					}
					//*/

					m_FuncNotify(m_dwListenerId, m_pArg);
				}
				else if (m_hWnd)
				{
